Deno.serve(async (req) => {
  try {
    ...
    const query = `SELECT Id, Name, FirstName, LastName, Email, Phone, MobilePhone, Title, Department, AccountId, Account.Name, MailingCity, MailingState, MailingCountry, csbs__Ownership__c, csbs__Credit_Score__c FROM Contact WHERE OwnerId = '${userId}' ORDER BY Name ASC LIMIT 2000`;
