...
Deno.serve(async (req) => {
  try {
    ...
    const query = `SELECT Id, Name, Phone, Fax, Website, BillingStreet, BillingCity, BillingState, BillingPostalCode, BillingCountry, ShippingStreet, ShippingCity, ShippingState, ShippingPostalCode, ShippingCountry, Type, Industry, NumberOfEmployees, Description, AccountNumber, Site, AnnualRevenue, CreatedDate, LastModifiedDate, RecordTypeId, RecordType.Name, CreatedBy.Name, Owner.Name, LastModifiedBy.Name, Parent.Name, csbs__Additional_Phone__c, csbs__Email__c, csbs__DBA__c, AccountSource, csbs__Type_of_Business__c, csbs__Product_Service_Sold__c, csbs__Federal_Tax_ID_Unencrypted__c, Sic, csbs__NAICS_Code__c, csbs__Active__c, csbs__Tier__c, csbs__Tier_Position__c, csbs__Priority_Lender__c, csbs__Emails_to_CC__c, csbs__Emails_to_BCC__c, csbs__Application_Industry__c, csbs__Minimum_Credit_Score__c, csbs__Maximum_Negative_Days__c, csbs__Minimum_Monthly_Deposit_Count__c, csbs__Maximum_NSFs__c, csbs__Minimum_Monthly_Deposit_Amount__c, csbs__Minimum_Average_Daily_Balance__c, csbs__Minimum_Months_in_Business__c, csbs__Maximum_Offer_Amount__c, csbs__Net_Offer_Percentage__c, csbs__Restricted_States__c, csbs__Restricted_Industries__c, csbs__Business_Start_Date__c, csbs__Business_Start_Date_Current_Ownership__c, csbs__Entity_Type__c, csbs__Franchise__c, csbs__Home_Based_Business__c, csbs__E_Commerce__c, csbs__Seasonal_Peak_Months__c FROM Account WHERE Id = '${accountId}'`;
