    ...
    try {
        ...
        if (oppResponse.ok) {
            ...
            if (opp.StageName?.toLowerCase() === 'approved') {
                const offerQuery = `SELECT Id, Name, csbs__Lender__c, csbs__Funded__c, csbs__Payment_Amount__c, csbs__Term__c FROM csbs__Offer__c WHERE csbs__Opportunity__c = '${opp.Id}' ORDER BY CreatedDate DESC`;
