...
Deno.serve(async (req) => {
  try {
    ...
    // Query all opportunities including declined ones
    const query = `SELECT Id, Name, StageName, Amount, CloseDate, Probability, AccountId, Account.Name, CreatedDate, LastModifiedDate, IsClosed FROM Opportunity WHERE OwnerId = '${userId}' AND (IsClosed = false OR (IsClosed = true AND StageName LIKE 'Closed - Declined%')) ORDER BY LastModifiedDate DESC`;
