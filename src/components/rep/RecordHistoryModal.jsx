        ...
        {loading ? (
          ...
          <div className="space-y-6">
            <div className="bg-slate-50 rounded-lg p-4">
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                <div>
                  ...
                  <p className="text-xs text-slate-500">{formatDate(history.record.createdDate)}</p>
                </div>
                <div>
                  <p className="text-slate-500 text-xs">Last Modified By</p>
                  <p className="font-medium text-green-600">{history.record.lastModifiedBy}</p>
                  <p className="text-xs text-slate-500">{formatDate(history.record.lastModifiedDate)}</p>
                </div>
              </div>
            </div>
            ...
            {history.fieldHistory.length > 0 && (
              <div>
                <h3 className="font-semibold text-slate-900 mb-3">
                  Field Changes ({history.fieldHistory.length})
                </h3>
                <div className="space-y-2 max-h-64 overflow-y-auto">
                    ...
                    <div key={idx} className="border border-slate-200 rounded-lg p-3 text-sm">
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <div>
                          ...
                          <p className="font-medium text-red-600">{change.oldValue || '(empty)'}</p>
                        </div>
                        <div>
                          <p className="text-xs text-slate-500">New</p>
                          <p className="font-medium text-green-600">{change.newValue || '(empty)'}</p>
                        </div>
                      </div>
                    </div>
                  ))}
