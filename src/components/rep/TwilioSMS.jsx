...
export default function TwilioSMS({ phoneNumber, recipientName, session, onMessageSent, color = 'bg-blue-600' }) {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm overflow-hidden flex flex-col h-96">
      ...
      <form onSubmit={handleSendSms} className="p-4 border-t border-slate-200 bg-white flex gap-2">
        <Input placeholder="Type message..." value={message} onChange={(e) => setMessage(e.target.value)} disabled={sending} className="text-sm" />
        <Button type="submit" disabled={sending || !message.trim()} className={`${color} hover:opacity-90 flex-shrink-0`}>
