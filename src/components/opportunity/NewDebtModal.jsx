            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-x-6 gap-y-4">
                <div className="space-y-4">
                  <div><Label htmlFor="creditor">Creditor</Label><div className="flex gap-2"><Select value={formData.creditorId} onValueChange={(value) => setFormData({ ...formData, creditorId: value })}><SelectTrigger><SelectValue placeholder="--None--" /></SelectTrigger><SelectContent>{lenders.map(lender => (<SelectItem key={lender.Id} value={lender.Id}>{lender.Name}</SelectItem>))}</SelectContent></Select><Button type="button" variant="outline" size="icon" onClick={() => setShowCreateLender(true)} title="Create New Lender"><Plus className="w-4 h-4" /></Button></div></div>
                  <div><Label htmlFor="balance">Balance</Label><Input id="balance" type="number" step="0.01" value={formData.balance} onChange={(e) => setFormData({ ...formData, balance: e.target.value })} /></div>
                  <div><Label htmlFor="lender">Lender</Label><Input id="lender" value={formData.lender} onChange={(e) => setFormData({ ...formData, lender: e.target.value })} /></div>
                  <div><Label htmlFor="estimatedMonthlyMCA">Estimated Monthly MCA Amount</Label><Input id="estimatedMonthlyMCA" type="number" step="0.01" value={formData.estimatedMonthlyMCA} onChange={(e) => setFormData({ ...formData, estimatedMonthlyMCA: e.target.value })} /></div>
                  <div className="flex items-center gap-2"><Checkbox id="openPosition" checked={formData.openPosition} onCheckedChange={(checked) => setFormData({ ...formData, openPosition: checked })} /><Label htmlFor="openPosition" className="cursor-pointer">Open Position</Label></div>
